...
// 로그 저장 파일 옵션
const dailyOptions = {
  level: 'verbose', // verbose 높은 단계의 로그만 기록
...
export const winstonLogger = WinstonModule.createLogger({
      ...
      format:
        env === 'production'
          ...
          : winston.format.combine(
              ...
              utilities.format.nestLike('TITAN', {
  ...
  format: winston.format.combine(
    ...
    winston.format.errors({ stack: true }), // 에러의 stack 정보를 포함
    winston.format.json(),
    winston.format.printf((info) => {
      return `${info.timestamp} - ${info.level} [${process.pid}]: ${info.message} ${info.stack || ''}`;
    }),
  ),
});
